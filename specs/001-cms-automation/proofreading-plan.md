...
## Phase 4: AI 語境驗證 (Contextual Validation) - 規劃中 📋

> **設計目標**: 從單純的拼寫檢查提升至「語境驗證（Contextual Validation）」層次，確保內容一致性與品牌專業度。

### 4.1 語句完整性優化 (Sentence Completeness)

**目標**: AI 應識別獨立段落的結構性質，主動建議修訂以提升完整性與權威感。

| 規則類型 | 描述 | 實現方式 | 狀態 |
|----------|------|----------|------|
| G1-001 | 導言段落識別 | AI + 語境分析 | 📋 規劃 |
| G1-002 | 結論段落優化 | AI + 模板匹配 | 📋 規劃 |
| G1-003 | 過渡段落建議 | AI + 結構分析 | 📋 規劃 |
...
### 4.2 符號一致性校驗 (Symbol Consistency)

**目標**: 定義統一的特殊符號解析規範，確保符號在「校對」與「發布」端的一致性。

| 規則 ID | 符號類型 | 標準形式 | 常見錯誤 | 狀態 |
|---------|----------|----------|----------|------|
| G2-001 | 波浪號 | ～（全形） | ~（半形） | 📋 規劃 |
| G2-002 | 破折號 | ——（兩個全形） | --（半形） | 📋 規劃 |
| G2-003 | 省略號 | ……（全形） | ...（半形） | 📋 規劃 |
| G2-004 | 引號 | 「」『』 | ""'' | 📋 規劃 |

### 4.3 地理位置與邏輯驗證 (Geographic & Logic Validation)

**目標**: 建立驗證層，防止 AI 產生的幻覺或邏輯矛盾導致誤導性地理資訊。

| 規則 ID | 驗證類型 | 描述 | 觸發條件 | 狀態 |
|---------|----------|------|----------|------|
| G3-001 | 地理名詞驗證 | 檢測邏輯矛盾 | AI 建議與地理邏輯衝突 | 📋 規劃 |
| G3-002 | AI 幻覺警告 | 語義衝突檢測 | AI 建議與原文語義差異大 | 📋 規劃 |
| G3-003 | 上下文一致性 | 前後文對照 | 前後段落資訊矛盾 | 📋 規劃 |

**案例**:
```
原文: 「位於上中西部的城市」
警告: [地理邏輯異常] 「上中西部」不是標準地理術語
建議操作: 保持原意 / 加註說明 / 自定義修改
```
...
- `frontend/docs/STATE_PERSISTENCE_FIX.md` - AI 語境驗證功能規劃
