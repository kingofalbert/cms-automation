...
### 2.17 AI 語境驗證警告標籤 (Contextual Validation Warning Labels) ⭐新增

> 用於標示需要編輯者手動驗證的 AI 建議，防止 AI 幻覺或邏輯矛盾導致錯誤發布。

**警告標籤類型**

| 類型 | 背景 | 邊框 | 文字 | Icon | 應用場景 |
|------|------|------|------|------|----------|
| `需手動驗證` | Warning-50 | Warning-400 | Warning-700 | AlertTriangle | AI 建議與原文差異較大 |
| `可能為 AI 幻覺` | Amber-50 | Amber-400 | Amber-700 | Sparkles | 檢測到潛在的 AI 生成錯誤 |
| `地理邏輯異常` | Error-50 | Error-400 | Error-700 | MapPin | 地理表述存在邏輯矛盾 |
| `符號格式異常` | Info-50 | Info-400 | Info-700 | Type | 檢測到非標準符號格式 |
| `結構優化建議` | Purple-50 | Purple-400 | Purple-700 | AlignLeft | 段落結構可優化 |
...
┌──────────────────────────────────────┐
│ ⚠️ 需手動驗證                        │  ← 標籤高度 28px
│ 地理名詞「上中西部」可能為 AI 幻覺    │  ← Tooltip 內容
└──────────────────────────────────────┘
```
...
│ [規則 A1-03] 統一用字                                  │
│ ┌─────────────────────────────────────────────────────┤
│ │ ⚠️ 可能為 AI 幻覺   📍 地理邏輯異常                 │  ← 警告標籤區
│ └─────────────────────────────────────────────────────┤
│ 原文: 「位於上中西部的城市...」                        │
│ 建議: 「位於中西部的城市...」                          │
│ Confidence: 0.72 (Claude)                              │
├───────────────────────────────────────────────────────┤
│ [接受建議] [保留原意] [自定義修改]                     │
│                                                        │
│ 💡 檢測到地理邏輯異常，建議人工確認                    │  ← 警告提示
└───────────────────────────────────────────────────────┘
```
...
1. **滑鼠懸停**
   - 顯示 Tooltip 說明警告原因
   - Tooltip 內容包含：警告類型 + 詳細說明 + 建議操作

2. **快捷鍵支持**
   - `W` - 跳轉到下一個警告項目
   - `Shift+W` - 跳轉到上一個警告項目
   - 快捷鍵提示：`快捷鍵: A 接受 | R 拒绝 | E 編輯 | W 警告導航`

3. **篩選功能**
   - 在問題列表頂部提供「僅顯示警告項目」Toggle
   - 可按警告類型過濾

**編輯者操作選項**

當觸發驗證警告時，除了標準的「接受/拒絕/自定義修改」外，額外提供：

| 操作 | 按鈕樣式 | 描述 |
|------|----------|------|
| 保持原意 | Secondary + Gray | 保留原文，標記為已驗證 |
| 加註說明 | Outline + Blue | 開啟備註輸入框 |
| 忽略警告 | Ghost + Gray | 繼續接受 AI 建議但記錄警告 |
...
- Textarea 最大 200 字符
- 加註內容存儲於 `proofreading_decisions.decision_rationale`
- 顯示字數計數器
