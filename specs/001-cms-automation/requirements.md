...
### 6. AI 語境驗證與內容邏輯 (Requirements: FR-056 to FR-062) ⭐新增

> **設計目標**: 從單純的拼寫檢查提升至「語境驗證（Contextual Validation）」層次，確保內容一致性與品牌專業度。

#### 6.1 語句完整性優化 (Sentence Completeness)

- [ ] **FR-056**: AI 應識別獨立段落的結構性質，並提供結構化修補建議
  - 識別導言段落、結論段落、過渡段落等
  - 對於不完整語句，主動建議修訂以提升完整性與權威感
  - **案例**: 「寵物諮詢獸醫」→ 建議修訂為「關於寵物：請務必諮詢獸醫」

- [ ] **FR-057**: AI 建議需標註修改類型（結構優化 / 語法修正 / 語義增強）
  - 前端顯示建議分類標籤
  - 編輯者可按類型過濾建議

#### 6.2 符號一致性校驗 (Symbol Consistency Validation)

- [ ] **FR-058**: 系統定義統一的特殊符號解析規範
  - 波浪號「~」、破折號「—」、省略號「……」等
  - 解析層確保符號在「校對」與「發布」端的一致性對應
  - 禁止解析過程中丟失或轉換符號
...
#### 6.3 地理位置與邏輯糾錯 (Geographic & Logic Validation Layer)

- [ ] **FR-060**: 建立地理名詞驗證規則
  - 維護常用地理名詞對照表（國家/省份/城市）
  - 檢測 AI 建議與通用地理邏輯的衝突
  - **案例**: 「上中西部/北部」等邏輯矛盾的地理表述需觸發警告

- [ ] **FR-061**: AI 幻覺（Hallucination）警告機制
  - 當 AI 建議與原始內容產生語義衝突時，觸發警告標籤
  - 警告標籤類型：`需手動驗證`、`可能為 AI 幻覺`、`地理邏輯異常`
  - 編輯者可選擇「加註說明」或「保持原意」

- [ ] **FR-062**: 驗證結果記錄與統計
  - 記錄觸發驗證警告的案例
  - 統計各類型警告的頻率，用於優化 AI 提示詞
  - 支持導出警告案例用於調優分析
