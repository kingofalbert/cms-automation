...
分析文章內容，必須**同時滿足以下三層門檻**才能生成FAQ：

**🚦 第一層：字數門檻（硬性條件）**
- 文章正文 ≥ 300字
- 不滿足 → `is_applicable = false`

**🚦 第二層：信息密度門檻（內容質量）**
文章必須包含**至少一類**以下信息：
- ✅ **可執行建議**: 具體做法/步驟/注意事項（如「每天服用2次」「先做A再做B」）
- ✅ **機制/原因解釋**: 為什麼會這樣/原理說明（如「因為XX導致YY」）
- ✅ **風險/禁忌/適用人群**: 誰不能用/什麼情況要注意
- ✅ **明確結論或觀點**: 清晰的判斷或建議（而非純陳述事實）

不滿足任何一類 → `is_applicable = false`，原因標註「內容信息密度不足」

**🚦 第三層：新聞體裁識別（防止事件播報誤加FAQ）**

若文章為**新聞類型**，必須滿足**至少一項**才允許加FAQ：
- ✅ 有明顯結構：H2/小標題 ≥ 2個
- ✅ 有引用/來源/數據 ≥ 1處（研究、機構、報告、調查、專家說法）
- ✅ 解釋性段落佔比高（出現「原因/影響/如何/建議/風險/分析/專家指出」等關鍵詞 ≥ 3次）
- ✅ 非單一事件播報（不是「發生了什麼」就結束，而是有延伸分析）
...
若判定為「純事件通報型新聞」（只告訴讀者發生了什麼，沒有解釋/建議/影響分析）→ `is_applicable = false`，原因標註「純事件通報，無延伸問答空間」

**輸出格式（適配性評估）**:
```json
"faq_assessment": {{
  "is_applicable": true,
  "reason": "本文內容深度足夠，包含可執行建議和機制解釋，讀者可能有多個延伸疑問",
  "target_pain_points": ["理解核心概念", "了解實際應用", "明確注意事項"],
  "article_type": "health_guide",  // 可選值見下方
  "word_count_check": true,
  "info_density_check": true,
  "news_genre_check": "not_news"  // 可選值: not_news, deep_analysis, event_report
}}
```

**article_type 可選值**:
- `health_guide`: 健康科普/指南類
- `disease_risk`: 疾病解讀/風險類
- `comparison`: 對比評測類
- `news_deep`: 新聞深度類
- `how_to`: 操作指南類
- `event_report`: 事件通報（不應加FAQ）
...
**問題設計原則（按文章類型的5W1H優先級）**:

⚠️ **重要：避免「為了凑框架而泛化」**
- 5W1H 是靈感池，不是必須全覆蓋
- 每種文章類型有最適合的問題方向，優先使用：

| 文章類型 | 優先問題方向 | 避免的問題 |
|---------|------------|----------|
| **健康科普/指南類** | Who(誰適合)、How(怎麼做)、When(何時做)、Why(為什麼) | Where(在哪裡)通常無意義 |
| **疾病解讀/風險類** | What(是什麼)、Who(誰高風險)、When(何時就醫) | Why(為什麼得病)可能太泛 |
| **對比評測類** | What(差異是什麼)、How(怎麼選)、Why(為什麼選這個) | When/Where通常不適用 |
| **新聞深度類** | What(是什麼)、Why(為何重要)、So what(影響)、How(怎麼應對) | 避免問「發生在哪裡」這類文中已明確的 |
...
2. **⚠️ 不得新增事實護欄（核心合規要求）**:
   - 答案**只能基於文章已有內容**，嚴禁杜撰或補充文章未提及的：
     - ❌ 具體劑量/頻率（如「每天3次，每次500mg」- 除非文章明確提及）
     - ❌ 具體時間週期（如「堅持2週見效」- 除非文章明確提及）
     - ❌ 百分比/統計數據（如「成功率達80%」- 除非文章明確提及）
     - ❌ 機構/研究引述（如「哈佛研究表明」- 除非文章明確提及）
   - 若文章未提供具體數據，使用模糊但準確的表述：
     - ✅「根據個人情況而定，建議諮詢專業人士」
     - ✅「效果因人而異，需持續觀察」
     - ✅「具體用量請遵醫囑或參考產品說明」
...
4. **字數黃金律**: **40-60字**（中文）/ 40-80 words（英文）
   - 這是Featured Snippet和AI Overviews的最佳抓取長度
   - 過短缺乏信息量，過長被截斷

5. **關鍵詞自然嵌入**: 將Focus Keyword自然融入至少1-2個FAQ答案中
...
- 建議諮詢專業人士的語句
- **避免過度確定的首句結論**：若文章本身不確定，答案也應保持適度模糊
...
{{
  ...
  "faq_assessment": {{
    ...
    "reason": "說明為何適合/不適合增加FAQ（需引用具體門檻判斷結果）",
    "target_pain_points": ["痛點1", "痛點2"],
    "article_type": "health_guide/disease_risk/comparison/news_deep/how_to/event_report",
    "word_count_check": true/false,
    "info_density_check": true/false,
    "news_genre_check": "not_news/deep_analysis/event_report"
  }},
  "faqs": [
...
3. **🚨 不得新增事實（核心合規）**: FAQ答案**嚴禁杜撰**文章未提及的具體數據、劑量、時間、百分比、研究引述
4. **長度控制**: 嚴格遵守FAQ答案40-60字限制（AI摘要最佳抓取長度）
5. **首句定論（但不過度確定）**: 第一句直接回答問題，但若文章本身不確定，答案也應保持適度模糊
6. **YMYL合規**: 健康類FAQ必須包含適當的安全警示和免責聲明
7. **三層門檻判斷**:
   - 第一層：字數 ≥ 300字
   - 第二層：信息密度（含可執行建議/機制解釋/風險禁忌/明確結論之一）
   - 第三層：新聞體裁識別（純事件通報不加FAQ）
8. **按類型選問題**: 根據 article_type 優先選擇對應的問題方向，避免為了凑5W1H而生成無意義問題
