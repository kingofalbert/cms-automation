/**
 * Meta Title Editor component with character count and AI generation indicator.
 */
...
export interface MetaTitleEditorProps {
  value: string;
...
export const MetaTitleEditor: React.FC<MetaTitleEditorProps> = ({
  value,
  onChange,
  isAIGenerated = false,
  onRegenerate,
  ...
  return (
    <div className={clsx('space-y-2', className)}>
      ...
      <input
        ...
        placeholder="输入 Meta Title（50-60 字符最佳）"
        className={clsx(
      ...
      <div className="flex items-center justify-between">
        ...
        />
        <p className="text-xs text-gray-500">Google 建议 50-60 字符</p>
      </div>

      {characterCount === 0 && (
        <p className="text-sm text-red-600">Meta Title 不能为空</p>
      )}
    </div>
  );
};
