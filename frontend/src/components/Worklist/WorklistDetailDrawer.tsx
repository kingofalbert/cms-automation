  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {/* WordPress Publishing Info - only shown for published status */}
        {resolvedStatus === 'published' && (data.wordpress_draft_url || item?.wordpress_draft_url) && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h3 className="font-medium text-blue-900 mb-3 flex items-center gap-2">
              ...
              {t('worklist.drawer.wordpressInfo')}
            </h3>
            <div className="space-y-3 text-sm">
              {/* WordPress Draft URL */}
              <div className="flex items-start justify-between">
                <span className="text-blue-700">{t('worklist.drawer.wordpressDraftUrl')}:</span>
                <a
                  href={data.wordpress_draft_url || item?.wordpress_draft_url || ''}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-blue-600 hover:text-blue-800 hover:underline flex items-center gap-1 max-w-[200px] truncate"
                >
                  <span className="truncate">{t('worklist.drawer.openInWordPress')}</span>
                  <ExternalLink className="w-3 h-3 flex-shrink-0" />
                </a>
              </div>

              {/* Post ID */}
              {(data.wordpress_post_id || item?.wordpress_post_id) && (
                <div className="flex justify-between">
                  <span className="text-blue-700">{t('worklist.drawer.wordpressPostId')}:</span>
              ...
              {/* Upload Time */}
              {(data.wordpress_draft_uploaded_at || item?.wordpress_draft_uploaded_at) && (
                <div className="flex justify-between">
                  <span className="text-blue-700">{t('worklist.drawer.uploadedAt')}:</span>
                  <span className="text-blue-900">
                    {formatDate(data.wordpress_draft_uploaded_at || item?.wordpress_draft_uploaded_at)}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="mt-4 flex gap-2">
              <Button
                ...
                onClick={() => window.open(data.wordpress_draft_url || item?.wordpress_draft_url || '', '_blank')}
              >
                <ExternalLink className="w-4 h-4 mr-1" />
                {t('worklist.drawer.editInWordPress')}
              </Button>
              <Button
                size="sm"
                variant="outline"
                onClick={() => {
                  const url = data.wordpress_draft_url || item?.wordpress_draft_url || '';
                  navigator.clipboard.writeText(url);
                }}
              >
                <Copy className="w-4 h-4 mr-1" />
                {t('worklist.drawer.copyUrl')}
              </Button>
            </div>
          </div>
        )}
