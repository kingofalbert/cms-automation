...
export const articlesAPI = {
  ...
  list: (params?: ArticleListParams) =>
    api.get<APIResponse<PaginatedResponse<Article>>>('/api/v1/articles', { params }),
  ...
  create: (data: ArticleCreateRequest) =>
    api.post<APIResponse<Article>>('/api/v1/articles', data),
  ...
  importCSV: (file: File, validateOnly: boolean = false) => {
    ...
    return api.post<APIResponse<ImportResult>>('/api/v1/articles/import/csv', formData, {
      headers: {
  ...
  importJSON: (file: File, validateOnly: boolean = false) => {
    ...
    return api.post<APIResponse<ImportResult>>('/api/v1/articles/import/json', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
