...
export const settingsAPI = {
   ...
   */
  getAll: () => api.get<APIResponse<Settings>>('/api/v1/settings'),

  /**
   * Update all settings at once.
   */
  updateAll: (settings: Partial<Settings>) =>
    api.put<APIResponse<Settings>>('/api/v1/settings', settings),

  /**
   * Get CMS configuration.
   */
  getCMSConfig: () => api.get<APIResponse<CMSConfig>>('/api/v1/settings/cms'),

  /**
   * Update CMS configuration.
   */
  updateCMSConfig: (config: Partial<CMSConfig>) =>
    api.put<APIResponse<CMSConfig>>('/api/v1/settings/cms', config),
  ...
  testCMSConnection: () =>
    ...
    >('/api/v1/settings/cms/test'),

  /**
   * Get provider configuration.
   */
  getProviderConfig: () =>
    api.get<APIResponse<ProviderConfig>>('/api/v1/settings/providers'),

  /**
   * Update provider configuration.
   */
  updateProviderConfig: (config: Partial<ProviderConfig>) =>
    api.put<APIResponse<ProviderConfig>>('/api/v1/settings/providers', config),
   ...
   */
  getCostLimits: () => api.get<APIResponse<CostLimits>>('/api/v1/settings/cost-limits'),

  /**
   * Update cost limits configuration.
   */
  updateCostLimits: (limits: Partial<CostLimits>) =>
    api.put<APIResponse<CostLimits>>('/api/v1/settings/cost-limits', limits),

  /**
   * Get screenshot retention configuration.
   */
  getScreenshotRetention: () =>
    api.get<APIResponse<ScreenshotRetention>>('/api/v1/settings/screenshots'),

  /**
   * Update screenshot retention configuration.
   */
  updateScreenshotRetention: (config: Partial<ScreenshotRetention>) =>
    api.put<APIResponse<ScreenshotRetention>>('/api/v1/settings/screenshots', config),
  ...
  cleanupScreenshots: () =>
    ...
    >('/api/v1/settings/screenshots/cleanup'),
   ...
   */
  resetToDefaults: () => api.post<APIResponse<Settings>>('/api/v1/settings/reset'),
  ...
  importSettings: (file: File) => {
    ...
    return api.post<APIResponse<Settings>>('/api/v1/settings/import', formData, {
      headers: {
